...
export const RSAEncryptDecrypt = () => {
    ...
    const encrypt = async () => {
        ...
        setResult(arrayBufferToHex(await rsaKey.encrypt(new TextEncoder().encode(data))));
    }
    ...
    const decrypt = async () => {
        ...
        setResult(new TextDecoder().decode(await rsaKey.decrypt(hexToArrayBuffer(data))));
    }

    return (
        <div>
            <textarea ref={keyRef} style={{ display: "block" }} placeholder="RSA Web Json Key" />
            <textarea ref={dataRef} style={{ display: "block" }} placeholder="Data" />
            <div>
