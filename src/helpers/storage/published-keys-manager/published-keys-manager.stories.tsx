...
export const Default: StoryFn = () => {
    ...
    const create = async () => {
        if(!initData) return setPublished(new PublishedKeysManager(new KeysIndex()))
        const buffer = base64ToArrayBuffer(initData);

        const storage = await PublishedKeysManager.import(buffer, new KeysIndex());
        ...
        <div>
            {
                !published ? (
                    <>
                        <div>
                            <textarea placeholder="Data to import Base64" value={initData} onChange={e => setInitData(e.target.value)} />
                        </div>
                    ...
                    <>
                        ...
                        <div style={{ marginTop: 30 }}>
                            ...
                            {exported && (
                                <>
                                    <h3>Exported</h3>
                                    <button onClick={() => navigator.clipboard.writeText(exported)}>Copy to clipboard</button>
                                    <div style={{ width: 500, wordWrap: 'break-word' }}>{exported}</div>
                                </>
                            )}
                        </div>
                    </>
