...
const ClientView: FC<{ client: ChatClient, connections: ConnectionsManager }> = ({ client, connections }) => {
    const [connectionIDS, setConnectionIDs] = useState<string[]>([])
    const [messages, setMessages] = useState<string[]>([]);

    const { controller: ConnectController, submit: connect, clear: clearConnectForm } = useController({
        fields: {
            addressKey: Str(true, ""),
            from: Str(true, ""),
        },
        async submit(data) {
            const key = await RSAEncryptionKey.fromSPKI(base64ToArrayBuffer(data.addressKey));
    ...
    const { controller: SendMessageController, submit: sendMessage, clear: clearSendMessageForm } = useController({
        fields: {
        ...
        async submit(data) {
            ...
            setMessages(p => p.concat([`TO ${data.connectionID}: ${data.message}`]))
    ...
    useEffect(() => {
        const sub = client.addEventListener(ev => {
            switch(ev.type) {
                ...
                case "message":
                    setMessages(p => p.concat([`FROM ${ev.id}: ${ev.message}`]))
                    break;

                case "newPendingConnection":
                    if(window.confirm(`Accept connection from "${ev.from}"?`)) {
    ...
    return (
        <div>
                    ...
                    <form onSubmit={e => (e.preventDefault(), connect())}>
                        <div>
                            <FieldConsumer field="addressKey">
                                {({ value, setValue }) => (
                                    <textarea placeholder="Address Base64 Key" value={value} onChange={e => setValue(e.target.value)} />
                                )}
                            </FieldConsumer>
                        </div>
                        <div>
            ...
            <div style={{ marginTop: "40px" }}>
                ...
                <ul>
                    {connectionIDS.map(i => <li key={i}>{i}</li>)}
                </ul>
            </div>
                        ...
                        <form onSubmit={e => (e.preventDefault(), sendMessage())}>
                            <div>
                                <FieldConsumer field="connectionID">
                                    {({ value, setValue }) => (
                                        <input placeholder="Connection ID" value={value} onChange={e => setValue(e.target.value)} />
                                    )}
                                </FieldConsumer>
                            </div>
                            <div>
                                <FieldConsumer field="message">
                                    {({ value, setValue }) => (
                                        <textarea placeholder="Message" value={value} onChange={e => setValue(e.target.value)} />
                                    )}
                                </FieldConsumer>
                            </div>
...
export const Default: StoryFn = () => {
    ...
    const publishedManager = useMemo(() => new PublishedKeysManager(keysIndex), [keysIndex]);
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            client = new ChatClient(protocol, connectionsManager, publishedManager, keysIndex);
            client.init();
                ...
                <FormProvider controller={ConnectionController}>
                    <form onSubmit={e => (e.preventDefault(), connect())}>
                        <FieldConsumer field="address">
                            {({ value, setValue }) => (
                                <input placeholder="Address" value={value ?? ""} onChange={e => setValue(e.target.value)} />
                            )}
                        </FieldConsumer>
                        <button type="submit">Connect</button>
                    </form>
            ...
            <div>
                ...
                {issuedKey && (
                    <div>
                        <h3>Key</h3>
                        {issuedKey}
                        <button onClick={() => setIssuedPublicKey(null)}>Clear</button>
                    </div>
                )}
            </div>
            {isLoading && <div>Loading...</div>}
            {
                client && <ClientView client={client} connections={connectionsManager} />
            }
        </div>
    )
}