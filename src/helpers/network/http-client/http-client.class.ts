...
import { arrayBufferToBlob, stringToArrayBuffer } from "../../arraybuffer-utils";
...
export class HTTPClient {
    ...
    async searchContact(name: string) {
        try {
            const result = await this.axios.post<Contact>("/contacts/find", { name })
    ...
    async updateContact(name: string, update: Partial<Contact>, key: SigningKey) {
        const updateData = [];
        ...
        const signature = await key.createSignature(stringToArrayBuffer(dataString));
    ...
    async deleteContact(name: string, signingKey: SigningKey) {
        const signature = await signingKey.createSignature(stringToArrayBuffer(name));
