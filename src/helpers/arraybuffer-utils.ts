...
export const arrayBufferToReadableStream = (src: ArrayBuffer) => new ReadableStream({
    type: 'bytes',
    start(controller) {
...
export const readableStreamToArrayBuffer = async (stream: ReadableStream<Uint8Array>) => {
    ...
    do {
        ...
        if(red.value) {
            if(!result) result = red.value;
                ...
                const merged: Uint8Array = new Uint8Array(result.byteLength + red.value.byteLength);
                merged.set(result, 0);
...
export const base64ToArrayBuffer = (src: string) => {
    ...
    const bytes = Uint8Array.from(raw.split("").map(char => char.charCodeAt(0)));
