...
export const Builder: StoryFn = () => {
    ...
    const update = () => {
        ...
        const str = Array.from(new Uint8Array(builder.getBuffer())).map(n => {
    ...
    return (
        <div>
            <input placeholder={builder ? "Data to write" : "Buffer length"} value={input} onChange={e => setInput(e.target.value)} />
            {
                !builder
                ? (
                    <div>
...
export const Reader: StoryFn = () => {
    ...
    const update = (buff = buffer, read = reader) => {
        ...
        const str = Array.from(new Uint8Array(buff, read.getCursor())).map(n => {
            const str = n.toString(16);
    ...
    return (
        <div>
            ...
            )}
            {
                ...
                ? (
                    <>
                        <input placeholder="Buffer in base64" value={input} onChange={e => setInput(e.target.value)} />
                        <div>
