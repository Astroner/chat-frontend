...
export default function SettingsPage() {
        ...
        async (enable: boolean) => {
            ...
            if (enable) {
                ...
                if (networkState.type !== 'READY') {
                    const { http: initHttp } = await network.init(
                        storage.connections,
                        storage.published,
                        storage.common,
                    );
                ...
                const { id: pushSubscriptionID } =
                    await http.addPushSubscription(
                        result.endpoint,
                        result.keys.p256dh,
                        result.keys.auth,
                    );
                ...
                if (pushSubscriptionID && networkState.type === 'READY') {
                    networkState.http.deletePushSubscription(
                        pushSubscriptionID,
                    );
                }
            }
    ...
    const toggleOnlineOnStartup = useCallback(
        async (value: boolean) => {
    ...
    useEffect(() => {
        ...
        const sub = storage.common.subscribe(() =>
            setOnlineOnStartup(storage.common.getData().onlineOnStartup),
        );
