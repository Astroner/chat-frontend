...
export const SlidablePanel: FC<SlidablePanelProps> = memo(({ onStateChange, ...props }) => {
    ...
    const api = useMemo<SlidableContextType>(() => ({
        ...
        touchMove: t => {
            ...
            setTouchPanelSpeed(p => {
                if(
                    nextSpeed / Math.abs(nextSpeed) !== p / Math.abs(p)
        ...
        touchEnd: () => {
            ...
            if(touchSpeed < -30 || (Math.abs(touchShift) > window.innerHeight / 2)) onStateChange(true)
