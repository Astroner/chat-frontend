import { arrayBufferToString, stringToArrayBuffer } from "@/src/helpers/arraybuffer-utils";
...
export type StorageState = { type: "IDLE" } | { type: "LOADING" } | {
    type: "READY",
    chats: ChatStorage
...
export class Storage {
    ...
    async init(password: string) {
        ...
        const aesKey = await AesGcmKey.fromPassword(password, Storage.PASSWORD_SALT);
        ...
        if(await this.env.hasData()) {
            ...
            const [ch, con, pubs] = await Promise.all([
                this.gzip.decompress(chatsData).then(data => new ChatStorage(JSON.parse(arrayBufferToString(data)))),
        ...
        for(const { key, connection } of connections.getAll()) {
            switch(connection.status) {
                case "requested":
                    this.keysIndex.addKey(key, connection.responseRSAPrivateKey)
        ...
        const dataToStore = {
            chats: await this.gzip.compress(stringToArrayBuffer(JSON.stringify(chats.getAll()))),
        ...
        this.sub = joinSubs(
            chats.subscribe(async () => {
                dataToStore.chats = await this.gzip.compress(stringToArrayBuffer(JSON.stringify(chats.getAll())));
                save();
            }),
