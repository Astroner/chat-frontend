...
export default function Home() {
    ...
    useEffect(() => {
        ...
        if(storageState.type === "READY" && isPanelOpen && network.getState().type === "IDLE") network.init(storageState.connections, storageState.published);
        ...
        <main className={cn.root}>
            <SlidablePanel open={isPanelOpen} onStateChange={setIsPanelOpen} >
                <div className={cn['panel-container']}>
                    <div className={cn['announcement--' + (announcementVisible ? 'visible' : 'hidden')]}>
                        Pull-up to connect
                    </div>
                    <div className={cn['quick-bar']}>
                        {networkState.type === "IDLE" && <div className={cn['connect-text']}>Not</div>}
                        <SlideAnchor>
                            <Button
                                color='orange'
                                icon={networkState.type === "READY" ? "network" : networkState.type === "CONNECTING" ? "hourglass" : "no-network"}
                                onTouchStart={touchButton}
                                onTouchEnd={releaseButton}
                            />
                        </SlideAnchor>
                       {networkState.type === "IDLE" && <div className={cn['connect-text']}>Connected</div>}
                    </div>
                    <div className={cn.networking}>
                        ...
                        }
                        {
                            networkState.type === "READY" && (
                                <>
                                    <Link className={cn['menu-button']} href={"/key/generate"}>
                                        <Button color='orange'>Generate invite QR</Button>
                                    </Link>
                                    <Link className={cn['menu-button']} href={"/keys"}>
                                        <Button color='orange'>Generated Invites</Button>
                                    </Link>
                                </>
                            )
                        }
