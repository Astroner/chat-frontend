...
export default function Home() {
    ...
    useEffect(() => {
        ...
        const handler = (message: MessageEvent) => {
            setMessages((p) =>
                p.concat([
                    `[SERVER] ${new Date(message.timeStamp).toISOString()} : ${message.data}`,
                ]),
            );
        };
    ...
    const send = (e: FormEvent) => {
        e.preventDefault();
        if (!textArea.current || !ws || textArea.current.value.length === 0)
            return;
        ...
        ws.send(message);
        setMessages((p) =>
            p.concat([`[CLIENT] ${new Date().toISOString()} : ${message}`]),
        );
    };
