...
import { ChatInfo, ChatState, ChatStorage, MessageOrigin } from '../helpers/storage/chat-storage.class';
...
import { arrayBufferToBase64, arrayBufferToString, base64ToArrayBuffer, stringToArrayBuffer } from '../helpers/arraybuffer-utils';
...
export default function Home() {
    ...
    const [storageEncryptionKey, setStorageEncryptionKey] = useState<AesGcmKey | null>(null);
    const gzip = useMemo(() =>  new GZip(), []);

    const [chatsStorage, setChatsStorage] = useState<ChatStorage | null>(null);
    const [connections, setConnections] = useState<ConnectionsManager | null>(null);
    const [published, setPublished] = useState<PublishedKeysManager | null>(null);
    
    const [chatClient, setChatClient] = useState<ChatClient | null>(null);

    const [displayedChats, setDisplayedChats] = useState<Array<{ title: string, id: string }>>([]);
    const [publishedKeysList, setPublishedKeysList] = useState<Array<{ id: string, timesUsed: number }>>([]);

    const [publishedKeyInfo, setPublishedKeyInfo] = useState<{ id: string, base64PublicKey: string } | null>(null);
    ...
    const loginController = useController({
        ...
        async submit({ password }) {
            const aes = await AesGcmKey.fromPassword(password, Uint8Array.from([
    ...
    const startConversationCOntroller = useController({
        ...
        async submit({ key: keySrc, from, to }, cb: (from: string, to: string, key: RSAEncryptionKey) => void) {
            const key = await RSAEncryptionKey.fromSPKI(base64ToArrayBuffer(keySrc));

            cb(from, to, key);
        }
    })

    const [connect, isConnecting, connection] = useAsyncCallback(async (hook) => {
    ...
    const call = async (from: string, to: string, key: RSAEncryptionKey) => {
        ...
        const { id: connectionID } = await chatClient.sendConnectionRequest(key, from);

        chatsStorage.createChat(to, connectionID);
    }

    const sendMessage = (message: string) => {
        if(!chatClient || !currentChatInfo || !connections || !chatsStorage) return;

        const connection = connections.getConnection(currentChatInfo.connectionID);
        ...
        (async () => {
            ...
            if(!stored) {
                ...
                const [chats, published, connections] = await Promise.all([
                    gzip.decompress(reader.readBytes()).then(data => new ChatStorage(JSON.parse(arrayBufferToString(data)))),
                ...
                for(const { key, connection } of connections.getAll()) {
                    switch(connection.status) {
                        ...
                        case "requested":
                            keysIndex.addKey(key, connection.responseRSAPrivateKey);
    ...
    useEffect(() => {
        if(!chatsStorage || !published || !connections || !storageEncryptionKey) return;
        ...
        (async () => {
            setDisplayedChats(chatsStorage.getAll().map(a => ({ title: a.title, id: a.id })))

            setPublishedKeysList(published.getAll().map(item => ({
                id: item.id,
                timesUsed: item.timesUsed
            })))
    
            const dataToStore = {
                messages: await gzip.compress(stringToArrayBuffer(JSON.stringify(chatsStorage.getAll()))),
            ...
            const save = async () => {
                const builder = new BufferBuilder(
                    6 + dataToStore.messages.byteLength + dataToStore.published.byteLength + dataToStore.connections.byteLength
                );
            ...
            chatsStorageSub = chatsStorage.subscribe(async () => {
                ...
                dataToStore.messages = await gzip.compress(stringToArrayBuffer(JSON.stringify(chatsStorage.getAll())))
            ...
            publishedSub = published.subscribe(async () => {
                ...
                setPublishedKeysList(publishedList.map(item => ({
                    id: item.id,
                    timesUsed: item.timesUsed
                })))
    ...
    useEffect(() => {
        ...
        const chatClient = new ChatClient(
            protocol, connections, published, keysIndex
        )
        chatClient.init();
        ...
        const sub = chatClient.addEventListener(event => {
            switch(event.type) {
                case "newPendingConnection": {
                    if(window.confirm(`New connection request from '${event.from}'`)) {
                        chatClient.acceptConnection(event.id);
                        const { id } = chatsStorage.createChat(event.from, event.id);
                    } else {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            setPublishedKeyInfo({
                id: info.id,
                base64PublicKey: arrayBufferToBase64(await info.publicKey.toSPKI())
            })
        })();
                    ...
                    <div>
                        <FormProvider controller={loginController.controller}>
                            <FieldConsumer field='password'>
                                {({ value, setValue }) => <Input placeholder='Storage Password' value={value ?? ""} onChange={setValue} />}
                            </FieldConsumer>
                            <Button variant='orange' onClick={loginController.submit}>Enter</Button>
                        </FormProvider>
                    </div>
                )
            }
                    ...
                    <div>
                        {
                            ...
                            : connection
                            ? "Connected"
                            : <Button variant='orange' onClick={connect}>Connect</Button>
                        }
                        {
                            ...
                            ? (
                                <ul>
                                    {displayedChats.map(info => (
                                        <li key={info.id} onClick={() => setScene({ name: "CHAT", id: info.id })}>
                                            {info.title}
                                        </li>
                                    ))}
                                </ul>
                        ...
                        {connection && (
                            <Button variant='orange' onClick={() => setScene({ name: "START_CONVERSATION" })}>Start conversation</Button>
                        )}
                        <Button variant='orange' onClick={() => setScene({ name: "PUBLISHED" })}>Published keys</Button>
                    </div>
                )
            }
            {
                scene.name === "PUBLISHED" && published && (
                    <div>
                            ...
                            ? (
                                ...
                                <ul>
                                    {publishedKeysList.map(item => (
                                        <li key={item.id} onClick={() => setScene({ name: 'PUBLISHED_KEY', id: item.id })}>
                        ...
                        <Button variant='orange' onClick={createKey}>Create new key</Button>
                        <Button variant='orange' onClick={() => setScene({name: 'CHATS'})}>Back To Chats</Button>
                    </div>
                )
            }
            {
                    ...
                    <div>
                        {
                            ...
                            : (
                                <>
                                    ...
                                    <div>Key:</div>
                                    <div style={{ width: '100%', wordWrap: 'break-word' }}>{publishedKeyInfo.base64PublicKey}</div>

                                    <Button variant='orange' onClick={copyToClipboard}>Copy to clipboard</Button>
                                </>
                            )
                        }
                        <Button variant='orange' onClick={() => setScene({name: 'PUBLISHED'})}>Back</Button>
                    </div>
                )
            }
            {
                scene.name === "START_CONVERSATION" && (
                    <div>
                        <FormProvider controller={startConversationCOntroller.controller}>
                            <form onSubmit={e => (e.preventDefault(), startConversationCOntroller.submit(call))}>
                                <FieldConsumer field='from'>
                                    {({ value, setValue, error }) => (
                                        <>
                                            <Input placeholder='From' value={value ?? ""} onChange={setValue} />
                                            {error}
                                        </>
                                    )}
                                </FieldConsumer>
                                <FieldConsumer field='to'>
                                    {({ value, setValue, error }) => (
                                        <>
                                            <Input placeholder='To' value={value ?? ""} onChange={setValue} />
                                            {error}
                                        </>
                                    )}
                                </FieldConsumer>
                                <FieldConsumer field='key'>
                                    {({ value, setValue }) => (
                                        <Input placeholder='Key' value={value ?? ""} onChange={setValue} />
                                    )}
                                </FieldConsumer>
                                <Button submit variant='orange'>Connect</Button>
                            </form>
                        </FormProvider>
                        <Button variant='orange' onClick={() => setScene({name: 'CHATS'})}>Back To Chats</Button>
                    </div>
                    ...
                    <div>
                        {
                            ...
                            : <div>
                                <Chat
                                    messages={currentChatInfo.messages.map(item => ({
                                        origin: item.origin === MessageOrigin.CLIENT ? "CLIENT" : "SERVER",
                                        text: item.text
                                    }))}
