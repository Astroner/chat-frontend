    ...
    const { controller, submit } = useController({
        ...
        async submit(data) {
            ...
            if(networkState.type !== "READY") {
                ...
                const { chat: Chat } = await network.init(storageState.connections, storageState.published);
                chat = Chat;
            ...
            const rsaKey = await RSAEncryptionKey.fromSPKI(base64ToArrayBuffer(key));

            const { id: connectionID } = await chat.sendConnectionRequest(rsaKey, data.from)
        ...
        <main className={cn.root}>
            <FormProvider controller={controller}>
                <form className={cn.container} onSubmit={e => (e.preventDefault(), submit())}>
                    ...
                    <Button submit color="orange">Connect</Button>
                </form>
            </FormProvider>
        </main>
    )
