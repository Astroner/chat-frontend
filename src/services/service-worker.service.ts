...
export type PushNotificationsState = { type: "denied" } | { type: "not-asked" } | {
    type: "granted",
    subscription: PushSubscription
}

export type ServiceWorkerServiceState = { type: "IDLE" } | { type: "LOADING" } | {
    type: "ACTIVE";
...
export class ServiceWorkerService {
    ...
    async pushSubscribe() {
        if(this.state.type !== "ACTIVE" || !this.registration) return "loading";
    ...
    async pushUnsubscribe() {
        if(this.state.type !== "ACTIVE" || this.state.pushNotifications.type !== "granted") return;
