    ...
    const initialKeys = await Promise.all(
        new Array(itemsNumber)
            ...
            .map(() => {
                ...
                const id = decoder.decode(buffer.slice(cursor, cursor + idLength));
                cursor += idLength;

                const keyLength = new Uint16Array(buffer.slice(cursor, cursor + 2));
                cursor += 2;
            ...
            .map(async item => {
                const key = await crypto.subtle.importKey('raw', item.keyMaterial, { name: 'HMAC', hash: 'SHA-256' }, false, ['sign', 'verify']);
...
const updateStorage = async () => {
    ...
    for(const key of keys.values()) {
        ...
        bytes.set(new Uint8Array(Uint16Array.from([key.bytes.length]).buffer), cursor);
        cursor += 2;
